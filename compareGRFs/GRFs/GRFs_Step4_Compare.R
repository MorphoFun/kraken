...
######## CONDUCTING MULTI-MODEL INFERENCE ##########

## Testing correlations amongst variables
...
Full.PeakNet.Corr.Pvalue <- cor.sig(Full.PeakNet[,1:7])
Full.PeakNet.Corr <- cor(Full.PeakNet[,1:7],Full.PeakNet[,1:7])

library(glmulti)
PercentStance.model <- lmer(PercentStance~group+(1|Ind),Full.PeakNet, verbose=T, REML=T)
PercentStance.LMER <- glmulti(PercentStance.model, level=2, crit="aicc")

attach(Full.PeakNet)
PercentStance.LMER <- glmulti(PercentStance~group*Species, level=2, fitfunc = lmer.glmulti, random = "+(1|Ind)", crit="aicc")
detach(Full.PeakNet)

######## CONDUCTING PHYLOGENETIC CONTRASTS ########
library(ape)
