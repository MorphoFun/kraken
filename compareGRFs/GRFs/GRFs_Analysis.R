...
GRFanalysis <- function(myData) {
  ...
  for (i in 1:length(Pec_Data)) {
    ...
    Pec_GRFs[[i]] <- voltToForce(Pec_Data[[i]], CalibInfo[i,3:5], zeroStart = Pec_VideoInfo[i,]$Pectoral.Start.Frame, lightStartFrame = Pec_VideoInfo[i,]$Light.Start, 
                                 startFrame = Pec_VideoInfo[i,]$Pectoral.Start.Frame, endFrame = Pec_VideoInfo[i,]$Pectoral.End.Frame, filename = Pec_VideoInfo[i,]$File.name, BW = Pec_VideoInfo[i,]$Body.Weight.kg)
    names(Pec_GRFs)[[i]] <- Pec_VideoInfo[i,]$File.name
    
    ## Filter the data
    saveAs[i] <- paste(Pec_VideoInfo[i,]$File.name, "_Pec_Filtered.pdf", sep = "")
    Pec_GRFs_Filtered[[i]] <- butterFilteR(Pec_GRFs[[i]], saveAs = saveAs[[i]], saveGraph = "yes")  
    names(Pec_GRFs_Filtered)[[i]] <- Pec_VideoInfo[i,]$File.name
    
    ## Calculating angles of GRF orientation
    Pec_GRFs_Filtered_dataset[[i]] <- GRFAngles(Pec_GRFs_Filtered[[i]]$GRF0Sum_filter_interp)
    names(Pec_GRFs_Filtered_dataset)[[i]] <- Pec_VideoInfo[i,]$File.name
    
    ## Converting to units of body weight
    BW_N[i] <- Pec_VideoInfo[i,]$Body.Weight.kg*9.8 # converting animal's body weight from kilograms to Newtons
    ...
    Pec_GRFs_Filtered_dataset_noOverlap[[i]] <- removeOverlaps(Pec_GRFs_Filtered_dataset[[i]], Pec_VideoInfo[i,2:3], Pec_VideoInfo[i,4:5], Pec_VideoInfo[i,]$Filming.Rate.Hz)
    ...
    ## Saving the graphs
    pdfSave[i] <- paste(Pec_VideoInfo[i,]$File.name, "_Pec_PostProcess.pdf", sep = "")
    pdf(pdfSave[[i]])
  ...
  for (i in 1:length(Pel_Data)) {
    ...
    ## Converting LabView output to GRFs ##
    Pel_GRFs[[i]] <- voltToForce(Pel_Data[[i]], CalibInfo[i,3:5], zeroStart = Pel_VideoInfo[i,]$Pelvic.Start.Frame, lightStartFrame = Pel_VideoInfo[i,]$Light.Start, 
                                 startFrame = Pel_VideoInfo[i,]$Pelvic.Start.Frame, endFrame = Pel_VideoInfo[i,]$Pelvic.End.Frame, filename = Pel_VideoInfo[i,]$File.name, BW = Pel_VideoInfo[i,]$Body.Weight.kg)
    names(Pel_GRFs)[[i]] <- Pel_VideoInfo[i,]$File.name
    
    ## Filter the data
    Pel_saveAs[i] <- paste(Pel_VideoInfo[i,]$File.name, "_Pel_Filtered.pdf", sep = "")
    Pel_GRFs_Filtered[[i]] <- butterFilteR(Pel_GRFs[[i]], saveAs = Pel_saveAs[[i]], saveGraph = "yes")  
    names(Pel_GRFs_Filtered)[[i]] <- Pel_VideoInfo[i,]$File.name
    
    ## Calculating angles of GRF orientation
    Pel_GRFs_Filtered_dataset[[i]] <- GRFAngles(Pel_GRFs_Filtered[[i]]$GRF0Sum_filter_interp)
    names(Pel_GRFs_Filtered_dataset)[[i]] <- Pel_VideoInfo[i,]$File.name
    
    ## Converting to units of body weight
    Pel_BW_N[i] <- Pel_VideoInfo[i,]$Body.Weight.kg*9.8 # converting animal's body weight from kilograms to Newtons
    ...
    ## Remove data when pectoral and pelvic appendages overlap
    Pel_GRFs_Filtered_dataset_noOverlap[[i]] <- removeOverlaps(Pel_GRFs_Filtered_dataset[[i]], Pel_VideoInfo[i,4:5], Pel_VideoInfo[i,2:3], Pel_VideoInfo[i,]$Filming.Rate.Hz)
    ...
    ## Saving the graphs
    pdfSave[i] <- paste(Pel_VideoInfo[i,]$File.name, "_Pel_PostProcess.pdf", sep = "")
    pdf(pdfSave[[i]])
