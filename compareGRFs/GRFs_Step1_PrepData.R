...
# 7-31-12: Made edits, so that the AP component is not converted by a negative one.  This is because we rotate the force plate, which results
# in the sign of the AP component changing (e.g., a posterior reading would be negative for the oppossum work, but negative for the salamanders.
...
setwd('/Users/SandyMKawano/Desktop/Research/Forelimbs on Force Plates/Force Data/Calibration Overviews')
CalibPath <- setwd('/Users/SandyMKawano/Desktop/Research/Forelimbs on Force Plates/Force Data/Calibration Overviews')
Calib.Ind <- list.files(CalibPath, pattern=".csv", full=TRUE)
Calib.Dates <- substring(Calib.Ind, 122,127)
Calib.Newest <- max(as.numeric(Calib.Dates))
Calib.Use <- Calib.Ind[substring(Calib.Ind, 122, 127) %in% Calib.Newest]
CalibFile <- data.frame(read.csv(Calib.Use, header=TRUE))
...
setwd('/Users/SandyMKawano/Desktop/Research/Forelimbs on Force Plates/Force Data')
